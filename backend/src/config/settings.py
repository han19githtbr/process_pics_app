...
def _default_bank_collection_name() -> str:
    """Coleção dedicada ao Banco de Imagens de Alta Qualidade, separada da coleção
    de histórico de segmentação de letras (mesmo esquema dev/produção)."""
    return 'high_quality_images_prod' if (os.getenv('ENVIRONMENT') or '').strip().lower() == 'production' else 'high_quality_images'
...
class Settings:
    ...
    # Banco de Imagens de Alta Qualidade (imagens melhoradas pelo QualityEnhancer)
    MONGODB_BANK_COLLECTION_NAME = (
        os.getenv('MONGODB_BANK_COLLECTION_NAME') or _default_bank_collection_name()
    ).strip()
