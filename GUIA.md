...
- **Implementação:**
  1. **Novo hook `frontend/src/hooks/useLivePreview.ts`:** sempre que a imagem carregada ou qualquer opção de `ProcessingOptions` muda, dispara (com debounce de 450ms, para não martelar o backend a cada pixel do slider) uma chamada real a `segmentImage(...)` — a mesma função usada pelo botão "Segmentar Imagem" — e guarda o resultado (`debugImage` com as bounding boxes) e um estado `loading`. Requisições desatualizadas (o usuário mexeu de novo antes da resposta chegar) são descartadas via um contador de request id, para nunca sobrescrever o preview com um resultado antigo.
  2. **`Segmenter.tsx`:** passou a usar `useLivePreview(sourceUpload.image, sourceUpload.file?.name, options)`. Calcula `previewDebugImage`/`previewLetterCount` (prioriza o item do histórico selecionado, senão o preview ao vivo, senão o último resultado de "Segmentar Imagem") e um contador `previewSignal` que é incrementado em `handleOptionsChange` sempre que existe uma imagem carregada.
  3. **Desktop:** o card "Imagem de Origem" (`ImageUploader`) agora recebe `debugImage={previewDebugImage}` e `focusDebugSignal={previewSignal}`. O `ImageUploader` ganhou um novo prop `focusDebugSignal`, que, ao mudar, alterna automaticamente a visualização para a aba "Debug" (a mesma aba que já existia, com as bounding boxes) — assim o efeito de cada ajuste aparece sozinho, sem precisar clicar manualmente em "Debug".
  4. **Mobile:** `ControlPanel.tsx` recebeu os novos props `previewImage` e `previewLoading`, além de um estado interno `activeControlKey` (atualizado via `onFocus`/`onPointerDown` nos sliders e `onFocus`/`onClick` nos toggles). Uma nova função `renderMobilePreview(key)` renderiza um bloco `.mobile-inline-preview` (imagem + indicador "Atualizando…") logo abaixo do controle correspondente, mas **somente** quando esse é o controle ativo. Esse bloco fica oculto por padrão (`display: none`) e só aparece dentro do breakpoint mobile já usado no projeto (`@media (max-width: 768px)`), em `ControlPanel.css`, para não duplicar a pré-visualização no Desktop (onde ela já aparece no card "Imagem de Origem").
- **Arquivos alterados/criados:** `frontend/src/hooks/useLivePreview.ts` (novo), `frontend/src/hooks/index.ts`, `frontend/src/components/Segmenter/Segmenter.tsx`, `frontend/src/components/ImageUploader/ImageUploader.tsx`, `frontend/src/components/ControlPanel/ControlPanel.tsx`, `frontend/src/components/ControlPanel/ControlPanel.css`.
